...
public class paymentServ extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try 
        {
            ...
            cr.add(Restrictions.eq("mid", Integer.parseInt(request.getParameter("id"))));
            ArrayList<Money> al = (ArrayList<Money>) cr.list();
            Money p = al.get(0);
            HttpSession hs = request.getSession();
            hs.setAttribute("al", p);
            tr.commit();

            RequestDispatcher rd = request.getRequestDispatcher("uspayment.jsp");
